...
func (i *SeriesIterator) Start() *Series {
	i.cursor = 0
	i.page = i.origin
	i.remaining = i.limit
...
func (i *SeriesIterator) Next() *Series {
	i.cursor++
...
func (i *SeriesIterator) fetch() *Series {
	// handle item limit
	if i.limit > 0 {
		if i.remaining <= 0 {
			return nil
	...
	// fetch the next page
	page, err := fetchManySeries(nextLink.request(nil, nil, NoEmbeds))
	if err != nil {
		return nil
	}
